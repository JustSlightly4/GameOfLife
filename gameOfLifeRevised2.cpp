...
unsigned char advanceGeneration(unsigned char *__restrict board, unsigned char *__restrict nextBoard, int sizeOfBoard) {
    ...
    for (int i = 1; i < sizeOfBoard-1; ++i) {
        ...
        unsigned char a0 = rowAbove[0];
        unsigned char a1 = rowAbove[1];
        unsigned char a2 = rowAbove[2];
        unsigned char a3 = rowCurrent[0];
        unsigned char a4 = rowCurrent[1];
        unsigned char a5 = rowCurrent[2];
        unsigned char a6 = rowBelow[0];
        unsigned char a7 = rowBelow[1];
        unsigned char a8 = rowBelow[2];

        //Second for loop that loops through one row
        for (int j = 1; j < sizeOfBoard-1; ++j) {

            //Determine the cells 'population score'
            int score = a0 + a1 + a2 + a3 + a5 + a6 + a7 + a8;
            ...
            unsigned char nextValue = (score == 3) || (a4 && score == 2);
            ...
            changed |= nextValue ^ a4;

            //Finally, set the nextBoard equivalent cell to be either 0 or 1
            rowNext[j] = nextValue;

            // shift window right
            a0 = a1; a1 = a2; a2 = rowAbove[j+2];
            a3 = a4; a4 = a5; a5 = rowCurrent[j+2];
            a6 = a7; a7 = a8; a8 = rowBelow[j+2];
        }
    }
    return changed;
